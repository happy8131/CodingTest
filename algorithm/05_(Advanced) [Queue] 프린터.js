...
function queuePrinter(bufferSize, capacities, documents) {
  // TODO: 여기에 코드를 작성합니다.
  //bufferSize  타입의 인쇄 작업 목록 크기
  //capacities 타입의 인쇄 작업 목록에 추가될 수 있는 최대 용량
  //documents 타입을 요소로 갖는 문서 크기가 나열된 배열
  let count = 0;
  let total = 0;
  let queue = []; //작업목록
  let curr = documents.shift(); //현재 문서

  for (let i = 0; i < bufferSize; i++) {
    queue.push(0); //작업목록이 만들어 졌다
  } //queue = [0,0] // 버퍼의 크기를 만들어준다

  queue.unshift(curr); //7,0,0
  queue.pop(); //7,0
  ...
  while (total) {
    total -= queue.pop();
    curr = documents.shift();
    if (total + curr <= capacities) {
      queue.unshift(curr);
      total += curr;
    } else {
      documents.unshift(curr); //4,5,6
      queue.unshift(0); //0,7
    }
    count++;
  }
